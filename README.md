...
알림은 **특보가 실제로 발령·해제됐을 때만** 나갑니다. 기온이 높다고 임의로 보내지 않습니다.
...
열대야는 정식 기상특보가 아니라 "밤사이 최저기온 25℃ 이상"이라는 통계 기준이라,
특보 API로는 잡히지 않고 단기예보 기온으로 직접 판정해야 합니다.
지금은 범위에서 뺐지만, 필요해지면 `summarize_temps`가 이미 예보를 파싱하고 있으므로
야간 구간 최저기온만 계산해 이벤트를 하나 더 만들면 됩니다.
